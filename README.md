...
# Warewulf Slurm Login Node Image

[![CI/CD](https://github.com/ualberta-rcg/warewulf-slurm-login/actions/workflows/deploy-warewulf-slurm-login.yml/badge.svg)](https://github.com/ualberta-rcg/warewulf-slurm-login/actions/workflows/deploy-warewulf-slurm-login.yml)
![Docker Pulls](https://img.shields.io/docker/pulls/rkhoja/warewulf-slurm-login?style=flat-square)
...
This repository contains a hardened **Slurm login node image** based on Ubuntu 24.04, built into a Docker container that is **Warewulf-compatible** and deployable on bare metal.

It's primarily used for imaging and provisioning Slurm login nodes using [Warewulf 4](https://warewulf.org) in high-performance computing clusters. The image includes Slurm client tools and CIS security hardening using the SCAP Security Guide.
...
* **Slurm client tools** (sbatch, srun, squeue, etc.)
...
This project includes a GitHub Actions workflow: `.github/workflows/deploy-warewulf-slurm-login.yml`.
...
2. Add the following:
   ...
   * `DOCKER_HUB_REPO` → your Docker Hub repo. In this case: *rkhoja/warewulf-slurm-login*
...
wwctl image import --build --force docker://rkhoja/warewulf-slurm-login:u24-6.8.0-31-slurm-24.11.6-1-nv570.148 login-gpu

# Import with kernel only (no NVIDIA)
wwctl image import --build --force docker://rkhoja/warewulf-slurm-login:u24-6.8.0-31-slurm-24.11.6-1 login-cpu

# Import without kernel (use host kernel)
wwctl image import --build --force docker://rkhoja/warewulf-slurm-login:slurm-24.11.6-1 login-minimal
```
